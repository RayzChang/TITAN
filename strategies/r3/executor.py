...
class MeanReversionOrderIntentBuilder:
    """Build Mean Reversion maker-limit intents without chasing price."""

    def __init__(self, cfg: R3Config):
        self.cfg = cfg
        entry_cfg = cfg.mean_reversion.entry_order
        self.order_type = str(entry_cfg.order_type)
        self.time_in_force = str(entry_cfg.time_in_force)
        self.timeout_5m_bars = int(entry_cfg.timeout_5m_bars)
        self.execution_timeframe = str(cfg.timeframes.execution)
    ...
    def build_entry_intent(
        ...
        *,
        ...
        return OrderIntent(
            ...
            quantity=float(quantity),
            reduce_only=False,
            reason_codes=["MEAN_REVERSION_MAKER_LIMIT_INTENT"],
            expires_at=expires_at,
            signal_id=signal_id,
        )
